...
public class TouchScroller {
  ...
  public static boolean isSupported() {
    if (isSupported == null) {
       ...
       */
      isSupported = detectTouchSupport() && !isAndroid3();
    }
    return isSupported;
  }
  ...
  /**
   * NOTE: Added by Brad
   * TODO: Remove default method set to true
   * This was from the TouchEvent class. The current
   * touch event class in GWT 2.2 does not include
   * this method, so I just copied / pasted it here.
   * @return
   */
  private static native boolean detectTouchSupport2() /*-{
	  var elem = document.createElement('div');
	  elem.setAttribute('ontouchstart', 'return;');
	  return (typeof elem.ontouchstart) == "function";
	}-*/;
  private static boolean detectTouchSupport() {
	  return true;
  }
  ...
  protected void onTouchEnd(TouchEvent<?> event) {
    ...
    // Added by Brad ... trying to enable onClick events...
    Point diff = startTouchPosition.minus(lastTouchPosition.getPoint());
    if(diff.getX()==0 && diff.getY()==0) {
			...
			NativeEvent clickEvent = Document.get().createMouseEvent(
					"click",true, true,0, x, y, x, y, false, false,
					false, false, NativeEvent.BUTTON_LEFT, target);
