...
public class IPhoneScroller4 {
	...
	public static class IPhoneScrollerConfig extends JavaScriptObject {
		...
		public final native void setBounce(boolean b) /*-{
			this.bounce = b;
		}-*/;
		...
		public final native void setLockDirection(boolean b) /*-{
			this.lockDirection = b;
		}-*/;
		...
		public final native void setFixedScrollbar(boolean b) /*-{
			this.fixedScrollbar = b;
		}-*/;

		public final native void setFadeScrollbar(boolean b) /*-{
			this.fadeScrollbar = b;
		}-*/;

		public final native void setHideScrollbar(boolean b) /*-{
			this.hideScrollbar = b;
		}-*/;

		public final native void setScrollbarClass(String scrollbarClass) /*-{
			this.scrollbarClass = scrollbarClass;
		}-*/;
	}

	public IPhoneScroller4(Widget w) {
		this(w, null);
	}

	public IPhoneScroller4(Widget w, IPhoneScrollerConfig config) {
	...
	protected native final void includeLightLibJSNI() /*-{
...
iScroll.prototype = {
	...
	_start: function (e) {
		...
		if (that.options.momentum) {
			matrix = new WebKitCSSMatrix($wnd.getComputedStyle(that.scroller, null).webkitTransform);
...
var has3d = 'WebKitCSSMatrix' in $wnd && 'm11' in new WebKitCSSMatrix(),
	hasTouch = 'ontouchstart' in $wnd,
	isIDevice = (/iphone|ipad/gi).test(navigator.appVersion),
	isAndroid = (/android/gi).test(navigator.appVersion),
	RESIZE_EV = 'onorientationchange' in $wnd ? 'orientationchange' : 'resize',
...
if (typeof exports !== 'undefined') exports.iScroll = iScroll;
	...
	protected native final void includeLibJSNI() /*-{
		...
		(function() {
			function iScroll(el, options) {
				var that = this, doc = $doc, div, i;

				that.wrapper = typeof el == 'object' ? el : doc
						.getElementById(el);
				that.wrapper.style.overflow = 'hidden';
				that.scroller = that.wrapper.children[0];
				...
				that.options = {
					...
					pullDownLabel : [ 'Pull down to refresh...',
							'Release to refresh...', 'Loading...' ],
					pullUpLabel : [ 'Pull up to refresh...',
							'Release to refresh...', 'Loading...' ],
					onPullDown : function() {
					},
					onPullUp : function() {
				...
				that.options.HWCompositing = that.options.HWCompositing
						&& hasCompositing;
				that.options.HWTransition = that.options.HWTransition
						&& hasCompositing;

				if (that.options.HWCompositing) {
					that.scroller.style.cssText += '-webkit-transition-property:-webkit-transform;-webkit-transform-origin:0 0;-webkit-transform:'
				...
				that.pullDownToRefresh = that.options.pullToRefresh == 'down'
						|| that.options.pullToRefresh == 'both';
				that.pullUpToRefresh = that.options.pullToRefresh == 'up'
						|| that.options.pullToRefresh == 'both';
			...
			iScroll.prototype = {
				...
				_scrollbar : function(dir) {
					...
					if (!that[dir + 'ScrollbarWrapper']) {
						...
						if (that.options.scrollbarClass) {
							...
							bar.style.cssText = 'position:absolute;z-index:100;' + (dir == 'h' ? 'height:7px;bottom:1px;left:2px;right:7px'
						...
						bar = doc.createElement('div');
						if (!that.options.scrollbarClass) {
							bar.style.cssText = 'position:absolute;z-index:100;background:rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.9);-webkit-background-clip:padding-box;-webkit-box-sizing:border-box;' + (dir == 'h' ? 'height:100%;-webkit-border-radius:4px 3px;'
					...
					if (dir == 'h') {
						that.hScrollbarSize = that.hScrollbarWrapper.clientWidth;
						that.hScrollbarIndicatorSize = m
								.max(
										m.round(that.hScrollbarSize
												* that.hScrollbarSize
												/ that.scrollerW), 8);
						that.hScrollbarIndicator.style.width = that.hScrollbarIndicatorSize + 'px';
						...
						that.vScrollbarSize = that.vScrollbarWrapper.clientHeight;
						that.vScrollbarIndicatorSize = m
								.max(
										m.round(that.vScrollbarSize
												* that.vScrollbarSize
												/ that.scrollerH), 8);
						that.vScrollbarIndicator.style.height = that.vScrollbarIndicatorSize + 'px';
				...
				_checkSize : function() {
					var that = this, scrollerW, scrollerH;

					if (that.moved || that.zoomed || !that.contentReady)
						return;

							scrollerW = m.round(that.scroller.offsetWidth
									* that.scale),
							scrollerH = m.round((that.scroller.offsetHeight
									- that.offsetBottom - that.offsetTop)
									* that.scale);

					if (scrollerW == that.scrollerW
							&& scrollerH == that.scrollerH)
						return;
				...
				_pos : function(x, y) {
					...
					that.scroller.style.webkitTransform = trnOpen + that.x
							+ 'px,' + that.y + 'px' + trnClose + ' scale('
							+ that.scale + ')';
				...
				_indicatorPos : function(dir, hidden) {
					...
					if (pos < 0) {
						...
						pos = that.options.fixedScrollbar ? that[dir + 'ScrollbarMaxScroll']
								: pos
				...
				_start : function(e) {
					...
					if (hasTouch && e.touches.length == 2 && that.options.zoom
							&& hasGesture && !that.zoomed) {
						that.originX = m.abs(e.touches[0].pageX
								+ e.touches[1].pageX - that.wrapperOffsetLeft
								* 2)
								/ 2 - that.x;
						that.originY = m.abs(e.touches[0].pageY
								+ e.touches[1].pageY - that.wrapperOffsetTop
								* 2)
								/ 2 - that.y;
					}
					...
					if (that.options.momentum) {
						...
						if (that.options.HWCompositing) {
							matrix = new WebKitCSSMatrix($wnd.getComputedStyle(
							...
							matrix = $wnd.getComputedStyle(that.scroller, null);
							if (that.x + 'px' != matrix.left
									|| that.y + 'px' != matrix.top) {
								that._unbind('webkitTransitionEnd');
								that._pos(matrix.left.replace(/[^0-9]/g) * 1,
										matrix.top.replace(/[^0-9]/g) * 1);
							}
						}
				...
				_end : function(e) {
					...
					var that = this, point = hasTouch ? e.changedTouches[0] : e, target, ev, momentumX = {
						...
						dist : 0,
						time : 0
					}, duration = e.timeStamp - that.startTime, newPosX = that.x, newPosY = that.y, newDuration, snap;
					...
					if (that.pullDownToRefresh && that.contentReady
							&& that.pullDownEl.className.match('flip')) {
					...
					if (that.pullUpToRefresh && that.contentReady
							&& that.pullUpEl.className.match('flip')) {
					...
					if (duration < 300 && that.options.momentum) {
						momentumX = newPosX ? that._momentum(newPosX
								- that.startX, duration, -that.x,
								that.scrollerW - that.wrapperW + that.x,
								that.options.bounce ? that.wrapperW : 0)
								: momentumX;
						momentumY = newPosY ? that._momentum(newPosY
								- that.startY, duration, -that.y,
								(that.maxScrollY < 0 ? that.scrollerH
										- that.wrapperH + that.y : 0),
					...
					if (momentumX.dist || momentumY.dist) {
						newDuration = m.max(m.max(momentumX.time,
								momentumY.time), 10);
					...
					if (resetX == that.x && resetY == that.y) {
						...
						if (that.zoomed) {
							if (that.options.onZoomEnd)
								that.options.onZoomEnd.call(that); // Execute custom code on scroll end
							that.zoomed = false;
						}
				...
				_timedScroll : function(destX, destY, runtime) {
					var that = this, startX = that.x, startY = that.y, startTime = (new Date)
							.getTime(), easeOut;
				...
				_gestChange : function(e) {
					var that = this, scale = that.scale * e.scale, x, y, relScale;
					...
					that.scroller.style.webkitTransform = trnOpen + x + 'px,'
							+ y + 'px' + trnClose + ' scale(' + scale + ')';
					that.lastScale = relScale;
				},

				_gestEnd : function(e) {
					var that = this, scale = that.scale, lastScale = that.lastScale;

					that.scale = scale * lastScale;
					if (that.scale < that.options.zoomMin + 0.05)
						that.scale = that.options.zoomMin;
					else if (that.scale > that.options.zoomMax - 0.05)
				...
				_wheel : function(e) {
					var that = this, deltaX = that.x + e.wheelDeltaX / 12, deltaY = that.y
							+ e.wheelDeltaY / 12;
				...
				// Utilities
				_momentum : function(dist, time, maxDistUpper, maxDistLower,
						size) {
					var that = this, deceleration = 0.0006, speed = m.abs(dist)
							/ time, newDist = (speed * speed)
							/ (2 * deceleration), newTime = 0, outsideDist = 0;

					// Proportinally reduce speed if we are outside of the boundaries 
					if (dist > 0 && newDist > maxDistUpper) {
						outsideDist = size
								/ (6 / (newDist / speed * deceleration));
						maxDistUpper = maxDistUpper + outsideDist;
						that.returnTime = 800 / size * outsideDist + 100;
						speed = speed * maxDistUpper / newDist;
						newDist = maxDistUpper;
				...
				_unbind : function(type, el) {
					(el || this.scroller)
							.removeEventListener(type, this, false);
				},
				...
				refresh : function() {
					var that = this, pos = 0, page = 0, i, l, els, oldHeight, offsets, loading;
					...
					that.wrapperW = that.wrapper.clientWidth;
					that.wrapperH = that.wrapper.clientHeight;
					that.scrollerW = m.round(that.scroller.offsetWidth
							* that.scale);
					that.scrollerH = m.round((that.scroller.offsetHeight
							- that.offsetBottom - that.offsetTop)
							* that.scale);
					that.maxScrollX = that.wrapperW - that.scrollerW;
					that.maxScrollY = that.wrapperH - that.scrollerH;
					...
					if (typeof that.options.snap == 'string') {
						...
						for (i = 0, l = els.length; i < l; i++) {
							...
							that.pagesX[i] = pos.x < that.maxScrollX ? that.maxScrollX
									: pos.x * that.scale;
							that.pagesY[i] = pos.y < that.maxScrollY ? that.maxScrollY
									: pos.y * that.scale;
						}
				...
				scrollTo : function(x, y, time, relative) {
					...
					time = !time
							|| (m.round(that.x) == m.round(x) && m
									.round(that.y) == m.round(y)) ? 0 : time;
				...
				scrollToElement : function(el, time) {
					...
					pos = that._offset(el);
					pos.x = pos.x > 0 ? 0
							: pos.x < that.maxScrollX ? that.maxScrollX : pos.x;
					pos.y = pos.y > 0 ? 0
							: pos.y < that.maxScrollY ? that.maxScrollY : pos.y;
					time = time === undefined ? m.max(m.abs(pos.x) * 2, m
							.abs(pos.y) * 2) : time;
			...
			var has3d = 'WebKitCSSMatrix' in $wnd
					&& 'm11' in new WebKitCSSMatrix(), hasTouch = 'ontouchstart' in $wnd, hasGesture = 'ongesturestart' in $wnd,
			//	hasHashChange = 'onhashchange' in $wnd,
			//	hasTransitionEnd = 'onwebkittransitionend' in $wnd,
			hasCompositing = 'WebKitTransitionEvent' in $wnd, isIDevice = (/iphone|ipad/gi)
					.test(navigator.appVersion), isAndroid = (/android/gi)
					.test(navigator.appVersion), RESIZE_EV = 'onorientationchange' in $wnd ? 'orientationchange'
					: 'resize', START_EV = hasTouch ? 'touchstart'
					: 'mousedown', MOVE_EV = hasTouch ? 'touchmove'
